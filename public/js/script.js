...
$('#myRange').on('input', function () {
  $('#spaar').text('$' + parseInt($(this).val()).toLocaleString())
  var value1 = parseInt($(this).val())

  // Map the value from rangeSlider1 to the range of rangeSlider2
  var min2 = 300000
  var max2 = 4000000
  var range2 = max2 - min2
  var value2 = min2 + (range2 * (value1 - 1500) / (81000 - 1500)) + 4850

  let percent = ((value1 - 1500) / (81000 - 1500)).toFixed(2) * 45
  if (percent > oldpercent) {
    if (((1 * percent) / 100).toFixed(0) > ((1 * oldpercent) / 100).toFixed(0)) { 
      let spans = $('#box_change span').length
 
      for (let i = spans; i < ((1 * percent) / 100).toFixed(0); i++) {
        
            $('#box_change').append('<span></span>')
   
      }
      console.log(((1 * percent) / 100).toFixed(0))
    }
  }
    ...
    if (((1 * percent) / 100).toFixed(0) < ((1 * oldpercent) / 100).toFixed(0)) {
      let spans = $('#box_change span').length
      for (let i = spans; i > ((1 * percent) / 100).toFixed(0); i--) {
        $('#box_change').find('span:last').remove()
      }
      console.log(((1 * percent) / 100).toFixed(0)) 
    }
  }
  ...
  oldpercent = percent
  $('#woningWaarde').text('$' + parseInt(value2).toLocaleString())
  // $('#myRange2').val(value2) 
...
// $('#myRange').on('input', function () {
//   bespaar = Number(((($('#myRange').val()) * 1)) * 0.5) - 0
//   makelaar = Number(((($('#myRange').val()) * 1)) * 0.5)
...
// document.getElementById("myclicked").addEventListener("click", color)

// function color() {

// var first=  document.getElementById("blue-color").style.color="#0008d7 "
// function ducument.
// var second= document.getElementById("orenge-color").style.color="#ffb000cf"

// }

$('.switch').click(function () {

  // if($('input[name="checker"]:checked'))   {
  //     // alert('dd')
  //     $('#orenge-color').removeClass('active2')
  //     $('#blue-color').addClass('activeOne')
  // } else{
  //     alert('dd')
  //     $('#blue-color').removeClass('activeOne')
  //     $('#orenge-color').addClass('active2')
  // }
...
$('.carouselNew-carousel-one').owlCarousel({
  margin: 10,
  center: true,
  loop: true,
  nav: true,
